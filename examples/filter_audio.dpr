...
function init_filter_graph(var graph: PAVFilterGraph; var src: PAVFilterContext;
  var sink: PAVFilterContext): Integer;
var
...
begin
  ...
  av_opt_set    (abuffer_ctx, 'ch_layout',      '5.0',                                AV_OPT_SEARCH_CHILDREN);
  ...
  snprintf(@options_str[0], SizeOf(options_str),
           'sample_fmts=%s:sample_rates=%d:ch_layouts=%s',
...
begin
  ...
  if planar <> 0 then
    ...
    planes   := 1;
  bps        := av_get_bytes_per_sample(AVSampleFormat(frame.format));
  if planar <> 0 then
  ...
  begin
    av_md5_init(md5);
    av_md5_sum(@checksum[0], frame.extended_data[i], plane_size);
...
begin
  ...
  av_channel_layout_from_mask(frame.ch_layout, INPUT_CHANNEL_LAYOUT);
  ...
  for i := 0 to 5 - 1 do
  begin
    data := PSingle(frame.extended_data[i]);
