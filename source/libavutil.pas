...
type
  ...
  pAVChannelOrder = ^AVChannelOrder;

  pAVChannelCustom = ^AVChannelCustom;
  AVChannelCustom = record
    id: AVChannel;
...
function av_malloc_array(nmemb: size_t; size: size_t): Pointer;
(* *
...
function av_mallocz_array(nmemb: size_t; size: size_t): Pointer;
(* *
...
var
  total: size_t;
begin
  if av_size_mult(nmemb, size, total) < 0 then
    Exit(nil);
  Result := av_malloc(total);
end;
...
begin
  if av_size_mult(nmemb, size, total) < 0 then
    Exit(nil);
  Result := av_mallocz(total);
end;

function av_calloc(nmemb: size_t; size: size_t): Pointer;
begin
  Result := av_mallocz_array(nmemb, size);
end;
{$ENDREGION}
{$REGION 'parseutils.h'}
